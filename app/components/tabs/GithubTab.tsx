...
export default function GithubTab({ github, contact, t }: GithubTabProps) {
    ...
    const username = useMemo(() => {
        ...
        if (fromCms) {
            if (fromCms.includes("github.com") || fromCms.includes("/")) {
                return parseGithubUsername(fromCms);
            }
            return fromCms;
        }
    ...
    useEffect(() => {
        const fetchRepos = async () => {
            ...
            try {
                const response = await fetch(`/api/github/repos?username=${encodeURIComponent(username)}&limit=${reposLimit}`);
                if (!response.ok) {
                    console.error(`Local API Error: ${response.status} for GitHub user ${username}`);
